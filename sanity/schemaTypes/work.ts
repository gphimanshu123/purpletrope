...
export default defineType({
	...
	fields: [
		...
		defineField({
			...
			title: 'Highlights',
			type: 'array',
			description: 'Keywords such as “Wall Painting” or “Graffiti” shown as badges.',
			of: [
				defineArrayMember({
					type: 'string',
					validation: (Rule) => Rule.trim().min(1).max(60),
				}),
			],
				...
				defineArrayMember({
					...
					fields: [
						defineField({
							...
							validation: (Rule) =>
								Rule.max(160).warning('Aim to keep alt text under 160 characters.'),
						}),
					],
				}),
			],
